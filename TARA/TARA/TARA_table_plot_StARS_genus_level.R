...
x <- readRDS("TARA_ref_genus_level.rds")
n <-dim(x)[1]
K <- dim(x)[2] - 1
genuses <- readRDS("TARA_gen_names_genus_level.rds")

results <- readRDS("TARA_StARS_para_MB_loop_genus_level.rds")

genus_1 <- matrix(rep(genuses[-(K + 1)], times = K), nrow = K, ncol = K)
genus_2 <- matrix(rep(genuses[-(K + 1)], each = K), nrow = K, ncol = K)
...
# Literature
pairs_lit <- readRDS("TARA_pairs_lit.rds")
...
# Intersection of the top-100 lists from the three methods
...
library(xtable)
xtable(data.frame(final_list[1:15, ]))

# Calculate the degree of vertex
sort(degree(i.graph.lit), decreasing = TRUE)
hist(degree(i.graph.lit))
ggplot(data = data.frame(degree(i.graph.lit)), aes(x = degree.i.graph.lit.)) + 
  geom_histogram(binwidth = 1) + 
  geom_density(aes(n=n, y=..density..*n))
mean(degree(i.graph.lit))
sd(degree(i.graph.lit))
# hist(degree(i.graph.lit))
i.graph.1.all <- graph.data.frame(Omega.1.StARS.sorted, directed = F, vertices = all_vertices)
sort(degree(i.graph.1.all), decreasing = TRUE)
hist(degree(i.graph.1.all))
ggplot(data = data.frame(degree(i.graph.1.all)), aes(x = degree.i.graph.1.all.)) + 
  geom_histogram(binwidth = 1) + 
  geom_density(aes(n=n, y=..density..*n))
i.graph.2.all <- graph.data.frame(Omega.2.StARS.sorted, directed = F, vertices = all_vertices)
sort(degree(i.graph.2.all), decreasing = TRUE)
# hist(degree(i.graph.2.all))
ggplot(data = data.frame(degree(i.graph.2.all)), aes(x = degree.i.graph.2.all.)) + 
  geom_histogram(binwidth = 1)
i.graph.3.all <- graph.data.frame(Omega.3.StARS.sorted, directed = F, vertices = all_vertices)
sort(degree(i.graph.3.all), decreasing = TRUE)
# hist(degree(i.graph.3.all))
ggplot(data = data.frame(degree(i.graph.3.all)), aes(x = degree.i.graph.3.all.)) + 
  geom_histogram(binwidth = 1)
...
ggplot(data = data_degree, aes(x = degree, color = method, linetype = method, fill = method)) + 
...
# Comparing sparsity. We only selected 30+ edges, while glasso selected 110+...
Omega.1 <- results$Omega.1
num_edges_1 <- (sum(Omega.1 != 0) - K) / 2
num_edges_1

Omega.2 <- results$Omega.2
num_edges_2 <- (sum(Omega.2 != 0) - K) / 2
num_edges_2

Omega.3 <- results$Omega.3
num_edges_3 <- (sum(Omega.3 != 0) - K) / 2
num_edges_3

total_possible_edges <- K * (K - 1) / 2
total_possible_edges

sparsity.3 = num_edges_3 / total_possible_edges
sparsity.2 = num_edges_2 / total_possible_edges
sparsity.1 = num_edges_1 / total_possible_edges
sparsity.1
sparsity.2
sparsity.3
