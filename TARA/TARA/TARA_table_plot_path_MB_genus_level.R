...
results <- readRDS("TARA_path_MB_genus_level.rds")

pairs_lit <- readRDS("TARA_pairs_lit.rds")

genus_1 <- matrix(rep(genuses, times = K), nrow = K, ncol = K)
genus_2 <- matrix(rep(genuses, each = K), nrow = K, ncol = K)
...
ggplot(data = d, aes(x = selected, y = selected.lit)) +
