...
truth_filter$species_org1

num_only <- data.frame(OTU_in_table %>% select(starts_with("TARA")))
num_gen <- data.frame(OTU_in_table %>% select(starts_with("TARA"), genus))
...
x = t(ref_comb)
saveRDS(x, "TARA_ref_genus_level.rds")
saveRDS(gen_names, "TARA_gen_names_genus_level.rds")
saveRDS(unique_pairs_data, "TARA_pairs_lit.rds")
