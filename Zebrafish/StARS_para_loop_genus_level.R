...
# change is.infected to FALSE for the uninfected group
is.infected <- TRUE
if(is.infected) {
  label.infected <- "infected" 
} else {
  label.infected <- "not_infected"
}

# Load screened and pre-processed data
x <- readRDS(paste(label.infected, "_genus_level.rds", sep = ""))
  ...
  Omegas.1.sub <- foreach (i_iter_sub = 1:iter_sub, combine = cbind, .export = "obj", .packages = c("huge", "propagate")) %dopar%
    {
      ...
      Omega.1.sub <- Compo_glasso_x_adj(x_sub, rho.list = rho, ratio.O = 100, ratio.z = 100)$Omega.1
      Omega.1.sub
    }
  stopCluster(cl)
...
saveRDS(results, paste(label.infected, "_StARS_para_loop_genus_level.rds", sep = "")) 
end_time <- proc.time()
